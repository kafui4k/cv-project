...
class PreviewForm extends Component {
  render() {
    ...
    const {generateResume, bioData, educationalExperiences, workExperiences} = this.props;
        ...
        <section>
          <div className='profile-header'>
            <span>
              ...
              <br/>{bioData.location && bioData.location}
            </span>
            <h3>{bioData.firstname && bioData.firstname} {bioData.lastname && bioData.lastname}</h3>
            <span>
              <ul>
          ...
          {educationalExperiences && educationalExperiences.map((eduExp, i) => (
              <div key={i + 1}>
                <div className='profile-education'>
                  <h3>{eduExp.location}</h3>
                  <h3>{eduExp.schoolAttended}</h3>
                  <h3>{eduExp.start} - {eduExp.end}</h3>
                </div>
                <div className='profile___edication___town___program'>
                    <ul>
                      <li>{eduExp.studyType} in {eduExp.studyTitle}</li>
                    </ul>
                </div>
              </div>
          ))}       
          ...
          {workExperiences && workExperiences.map((workExp, i) => (
            <div key={i + 1}>
              ...
              <div>
                <span>{workExp.location} | {workExp.jobType}</span>
                <ul>
